...
if [ "$(sysrc -n dbus_enable)" != "YES" ]; then      # 检查 dbus 是否启用
        echo "dbus should be enable. Continue?(Y|n)"  # 提示用户是否启用
        read answer
        case $answer in
            [Nn][Oo]|[Nn])
...
case $answer in
	[Nn][Oo]|[Nn])
		...
		;;
	[Yy][Ee][Ss]|[Yy]|"")
		echo "compat.linux.osrelease=6.1.45" >> /etc/sysctl.conf   # 写入 sysctl 配置
		sysctl compat.linux.osrelease=6.1.45                     # 应用 sysctl 配置
                ;;
esac
