...
作用：允许任意外部设备通过 TCP 访问服务器指定端口（SSH、HTTP、HTTPS 等）。
...
作用：允许服务器访问 DNS（53 端口）和 DHCP（67 端口）服务器，并保持状态。
...
作用：允许 UDP 端口 33434-34500 用于 traceroute，并保持状态。
...
作用：将本机 em0 网卡的 8080 端口流量转发到本机 80 端口。
